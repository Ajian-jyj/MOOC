# -*- encoding: utf-8 -*-

import requests
import pathlib
import json
import random
from time import sleep
import re


class Mooc():

    def __init__(self):
        self.mob_token = ""

    def filejudge(self, path):
        dirpath = pathlib.Path(path)
        if dirpath.exists() == False:
            dirpath.mkdir()

    def get_courseId(self, cid, tid):
        Paperdict = {}
        data = {
            "mob-token": self.mob_token,
            "tid": tid,
            "cid": cid,
        }
        url = "https://www.icourse163.org/mob/course/courseLearn/v1"
        response = requests.post(url=url, data=data)
        results = response.json()['results']
        termDto = results['termDto']
        chapters = termDto["chapters"]
        for chapter in chapters:
            chaptername = chapter["name"]
            contentType = chapter["contentType"]
            Paperdict[chaptername] = {}
            if contentType == 2:
                name = chapter["name"]
                id = chapter["id"]
                Paperdict[chaptername][name] = id
            else:
                lessons = chapter["lessons"]
                quizs = chapter["quizs"]
                if lessons != None or lessons != []:
                    for lesson in lessons:
                        units = lesson["units"]
                        for unit in units:
                            unitname = unit["name"]
                            contentId = unit["contentId"]
                            contentType = unit["contentType"]
                            if contentType == 5:
                                Paperdict[chaptername][unitname] = contentId

                if quizs == None or quizs == []:
                    pass
                else:
                    quizs = quizs
                    for quiz in quizs:
                        quizname = quiz["name"]
                        contentId = quiz["contentId"]
                        Paperdict[chaptername][quizname] = contentId
            if Paperdict[chaptername] == {}:
                del Paperdict[chaptername]
        exams = termDto['exams']
        if exams != None or exams == []:
            Paperdict['exams'] = {}
            for exam in exams:
                name = exam['name']
                objectTestId = exam['objectTestId']
                if objectTestId != None:
                    Paperdict['exams'][name] = objectTestId
                else:
                    subjectTestId = exam['subjectTestId']
                    Paperdict['exams'][name] = subjectTestId
            if Paperdict['exams'] == {}:
                del Paperdict['exams']
        courseDto = results['courseDto']
        courseName = courseDto['name']
        schoolName = courseDto['schoolName']
        return schoolName, courseName, Paperdict

    def get_answer(self, id):
        data = {
            "mob-token": "",
            "testId": id,
            "isExercise": "true",
            "withStdAnswerAndAnalyse": "true"
        }
        headers = {
            "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
            'user-agent': "Dalvik/2.1.txt.0 (Linux; U; Android 8.1.txt.0; Pixel Build/OPM4.171019.021.P1)",
        }
        url = "https://www.icourse163.org/mob/course/paperDetail/v1"
        response = requests.post(url, data=data, headers=headers)
        answer = response.json()
        return answer

    def download(self, Paperdict, path):
        for chaptername, dic in Paperdict.items():
            # print(chaptername)
            chaptername = re.sub("[\/\\\:\*\?\"\<\>\|]|\x0b|：|\t","",chaptername).strip()
            filepath = f"{path}/{chaptername}"
            pathjudge = pathlib.Path(filepath)
            if pathjudge.exists() == False:
                pathjudge.mkdir()
            for name, id in dic.items():
                filePath = f"{filepath}/{name}.txt"
                pathJudge = pathlib.Path(filePath)
                if pathJudge.exists() == False:
                    answer = self.get_answer(id)
                    try:
                        if answer != None:
                            with open(filePath, 'w') as f:
                                f.write(json.dumps(answer, indent=4))
                            print(f' {filePath} ----->保存成功！')
                        else:
                            print(f' {name} 无答案！')
                    except:
                        print(f' {name} 无答案！')
                    sleep(random.random())
                else:
                    print(f' {filePath} 已经保存')

    def runId(self, cid, tid):
        schoolName, courseName, Paperdict = self.get_courseId(cid=cid, tid=tid)
        if Paperdict != {}:
            print(f' {courseName} {schoolName} 开始处理！\n')
            path = f"download/{courseName}_{schoolName}_{cid}_{tid}"
            self.filejudge('download')
            self.filejudge(path)
            self.download(Paperdict, path)
            print(f'\n {courseName} {schoolName} 处理完毕！')
        else:
            print(f' {courseName} {schoolName} 暂时还没有测验！')

    def runLink(self, link):
        cid, tid = re.findall('.*?/\w+-(\d+)\?tid=(\d+)', link)[0]
        schoolName, courseName, Paperdict = self.get_courseId(cid=cid, tid=tid)
        if Paperdict != {}:
            print(f' {courseName} {schoolName} 开始处理！\n')
            path = f"download/{courseName}_{schoolName}"
            self.filejudge('download')
            self.filejudge(path)
            self.download(Paperdict, path)
            print(f'\n {courseName} {schoolName} 处理完毕！')
        else:
            print(f' {courseName} {schoolName} 暂时还没有测验！')
